        ...
        run: async ({ file_path, view_range }) => {
            ...
            try {
                ...
                if (st.size > READ_MAX_BYTES) {
                    throw new ToolError(`read: ${file_path} is ${st.size} bytes, exceeds ${READ_MAX_BYTES}-byte limit. ` +
        ...
        run: async ({ file_path, old_string, new_string, replace_all }) => {
            ...
            try {
                ...
                if (st.size > EDIT_MAX_BYTES) {
                    throw new ToolError(`edit: ${file_path} is ${st.size} bytes, exceeds ${EDIT_MAX_BYTES}-byte limit. ` +
